""" Utilitaires pour la modelisation et la simulation (py3). 
      References:
        * http://python.org/
...
class Moment(object):
    ...
    q2FR = ["Jour", "Nuit"]
    q3FR = ["Jour", "Soir", "Nuit"]
    q3EN = ["Day", "Evening", "Night"]
    q = q2FR
    dq2FR = {"Jour":0, "Nuit":1}
    dq3FR = {"Jour":0, "Soir":1, "Nuit":2}
    dq3EN = {"Day":0, "Evening":1, "Night":2}
    dq = dq2FR
    # constantes utiles
    ...
    def __init__(self):
        ...
        self.Jdepart = "Dimanche"
        self.Qnb = 2    # nb de Q par jour (doit etre coherent avec Moment.q)
        self.Qmj = self.convertHM2M(0.0) # debut du quart de jour (h.m)
        ...
        self.Dx = True; self.Mx = False; self.Fx = False

    def update(self):
        """ Avance de dt dans le temps et update des variables associees. """
        self.Jtick = False; self.Qtick = False; self.MINtick = False
        self.Htick = False; self.Q24tick = False; self.TICKtick = False
        self.Dx = False; self.Mx = False; self.Fx = False
        self.SECtot += self.dt
        if (self.SECtot % 60 == 0):
            ...
            if ((self.MINnow - self.Qmj) % self.Qmin == 0):
                ...
                self.Qtot += 1
                self.Dx = True
            if ((self.MINnow - self.Qmj) % self.Qmin == 360): # 6h apres debut du quart, i.e. mi-quart 
                if not self.Dx: self.Mx = True
            if ((self.MINnow - self.Qmj) % self.Qmin == 660): # 11h apres debut du quart, i.e. juste av fin de quart 
                self.Fx = True
    ...
    def strHMSnow(self):
        """ Return HMS du moment actuel via une string. """
        m = int(self.MINnow % 60)
        if m < 10: sm = "0" + str(m)
        else: sm = str(m)
        s = int(self.SECtot % 60)
        if s < 10: ss = "0" + str(s)
        else: ss = str(s)
        return str(int(self.MINnow / 60)) + "h" + sm + "m" + ss + "s"

    def strHMnow(self):
        """ Return HM du moment actuel via une string. """
        m = int(self.MINnow % 60)
        if m < 10: sm = "0" + str(m)
        else: sm = str(m)
        return str(int(self.MINnow / 60)) + "h" + sm

    def strNow(self):
        """ Return Jour, jours total, HM du moment actuel via une string. """
...
class StatVA(object):
    ...
    def strVAint(self):
        """ Retourne une string avec les resultats convertis en integer. """
        s = self.nom + " ---"
        if (self.nb > 0):
            s = "  " + self.nom + "(moy/max): " + str(int(self.moy())) + "/" + str(int(self.max))
        return s
...
class Datastore(object):
    """ Conserve des donnes sous forme de string et les sauve dans un fichier csv. 
    En respectant les conventions du format csv (comma-separated values), 
    c'est compatible avec les chiffrier. 
    Ref: http://fr.wikipedia.org/wiki/Comma-separated_values
    """
