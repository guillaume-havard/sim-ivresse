...
class GenDistQueue(GenU01MRG):
    """ Generateur selon la distribution de moyenne m:
        {12*(m/2),m,m,2m,3m,4m} de 17 valeurs. 
    """

    def __init__(self, m, rac = 0):
        GenU01MRG.__init__(self, rac)
        self.n = 17
        if m > 10:
            self.m = int(m); self.m2 = int(m / 2)
        else:
            self.m = m; self.m2 = m / 2
        self.eps = 1.0 / float(self.n)
